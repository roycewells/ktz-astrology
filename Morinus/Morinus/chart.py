...
class Chart:
	...
	def __init__(self, name, male, time, place, htype, notes, options, full = True, proftype = 0, nolat=False):
		...
		# print "cal",time.cal
		# print "zt",time.zt
		# print "plus",time.plus
		# print "zh",time.zh
		# print "zm",time.zm
		# print "time.daylightsaving",time.daylightsaving
		
		# print "place.deglon", place.deglon
		# print "place.minlon", place.minlon
		# print "place.seclon", place.seclon
		# print "place.east", place.east
		# print "place.deglat", place.deglat
		# print "place.minlat", place.minlat
		# print "place.seclat", place.seclat
		# print "place.north", place.north
		# print "place.altitude", place.altitude
...
#         my addition             #  
###################################
	def get_aspmatrix_data(self,sig,chart):
		field_names = ['planet1','planet2','degrees']
		f=0;
				...
				if j > i:
					if self.aspmatrix[j][i].typ != Chart.NONE:
						...
						if self.aspmatrix[j][i].appl:
							appltxt = 'appl'
						# print '%s - %s: type=%d diff=%f %s par=%s %s\n' % (planets[i], planets[j], self.aspmatrix[j][i].typ, self.aspmatrix[j][i].dif, appltxt, partxt[plel], extxt)
						tmp = common.common.Aspects[chart.aspmatrix[j][i].typ]
						if(tmp=="S" or tmp=="R" or tmp=="P" or tmp=="W"):
							sig[f] = {
								field_names[0] : planets[i],
								field_names[1] : planets[j],
								field_names[2] : tmp
							}
							f=f+1
