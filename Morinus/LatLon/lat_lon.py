...
class LatLon:
    ...
    def get_lat(self):
        hemisphere = True
        if self.lat.get_hemisphere() == 'S':
            hemisphere = False
        return (f(self.lat.degree), f(self.lat.minute), f(self.lat.second), hemisphere)

    def get_lon(self):
        hemisphere = True
        if self.lon.get_hemisphere() == 'W':
            hemisphere = False
        return (f(self.lon.degree), f(self.lon.minute), f(self.lon.second), hemisphere)
